                ...
                child: IconButton(
                  onPressed: () {
                    final enriched = _filteredCalls.map((c) {
                      final uid = (c['staff_id'] ?? c['userId'] ?? c['user_id'] ?? c['createdBy'] ?? '').toString();
                      
                      // Precise User Name Resolution
                      String resolvedName = _findUserName(uid);
                      if (resolvedName.startsWith('User (') && c['userName'] != null) {
                         // If lookup failed but document has a name, use it
                         resolvedName = c['userName'].toString();
                         // Clean up if it's still a User(ID) format
                         if (resolvedName.startsWith('User (')) {
                            final match = RegExp(r'User \(([^)]+)\)').firstMatch(resolvedName);
                            if (match != null) {
                               final innerId = match.group(1);
                               final lookUp = _findUserName(innerId);
                               if (!lookUp.startsWith('User (')) resolvedName = lookUp;
                            }
                         }
                      }
                      ...
                      return {
                        ...c,
                        'userName': resolvedName, // Use resolved name
                    ...
                    if (enriched.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('No call logs found to export.'), backgroundColor: Colors.orange)
                      );
                      return;
                    }
                    
                    final dateRangeStr = '${DateFormat('MMM d').format(_startDate)}-${DateFormat('MMM d').format(_endDate)}';
                    ExportService.exportCallsToCsv(enriched, 'CallLogs_$dateRangeStr');
                  },
                  icon: const Icon(Icons.file_download_outlined, color: AppColors.primary),
                  tooltip: 'Download Call Logs',
                ),
              ),
            ],
          ),
