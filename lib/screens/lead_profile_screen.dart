...
class _LeadProfileScreenState extends State<LeadProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        stream: FirebaseFirestore.instance.collection('leads').doc(widget.leadId).snapshots(),
    ...
    return Column(
      children: [
        IconButton.filled(
          onPressed: onTap,
          icon: Icon(icon),
          style: IconButton.styleFrom(backgroundColor: color.withOpacity(0.1), foregroundColor: color),
        ),
        const SizedBox(height: 4),
  ...
  Widget _buildLeadMetaInfo(LeadModel lead) {
    return Column(
      children: [
        ...
        const Divider(),
        _buildInfoRow(Icons.access_time, "Created", DateFormat('MMM d, yyyy').format(lead.createdAt)),
      ],
    );
  }
  ...
  Widget _buildStatusPipeline(LeadModel lead, LeadProvider provider) {
    final statuses = ['New Inquiry', 'Contacted', 'Interested', 'Demo Scheduled', 'Converted', 'Not Interested'];
    
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Lead Status", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              return Padding(
                ...
                child: ChoiceChip(
                  ...
                  labelStyle: TextStyle(
                    color: isCurrent ? _getStatusColor(status) : AppColors.textSecondary,
      ...
      child: Column(
        children: [
          ...
          SizedBox(
            ...
            child: TabBarView(
              children: [
                _buildActivityTimeline(lead),
                _buildNotesList(lead),
              ],
            ),
          ),
        ],
  ...
  Widget _buildActivityTimeline(LeadModel lead) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final activities = snapshot.data!.docs.map((doc) => ActivityModel.fromFirestore(doc)).toList();
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(activity.activityType, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  Text(
              ...
              const SizedBox(height: 4),
              Text(activity.description, style: TextStyle(color: AppColors.textSecondary)),
              const SizedBox(height: 20),
            ],
          ),
        ),
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final notes = snapshot.data!.docs.map((doc) => NoteModel.fromFirestore(doc)).toList();
          ...
          ElevatedButton(
            onPressed: () {
              if (_noteController.text.isNotEmpty) {
                context.read<LeadProvider>().addNote(leadId, _noteController.text);
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      case 'New Inquiry': return Colors.blue;
      case 'Contacted': return Colors.orange;
      case 'Interested': return Colors.purple;
      case 'Demo Scheduled': return Colors.teal;
